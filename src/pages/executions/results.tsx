...
const ExecutionResults = () => {
  ...
  const viewMobile = useCallback(() => {
    navigate(`/executions/${uuid}/mobile`);
  }, [uuid, navigate]);

  const viewDesktop = useCallback(() => {
    navigate(`/executions/${uuid}/desktop`);
  }, [uuid, navigate]);

  const records = useMemo(() => {
    if (data) {
      return { desktop: data.results.heartbeats.find((item: any) => item.mode === 1), mobile: data.results.heartbeats.find((item: any) => item.mode === 0) };
    }
  }, [data]);
                ...
                {scores.length > 1 ? (
                  <div className='my-4'>
                    ...
                    <div className='my-4 flex gap-4'>
                      <Button icon={<DocumentSearchRegular />} onClick={viewMobile}>
                        View Mobile Details
                      </Button>
                      <Button icon={<DocumentSearchRegular />} onClick={viewDesktop}>
