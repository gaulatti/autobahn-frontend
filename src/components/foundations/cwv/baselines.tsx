...
const UpdateBaselinePopover = ({
  ...
  const handleSave = useCallback(
    (e: React.SyntheticEvent<HTMLFormElement>) => {
      ...
      sendRequest(Method.POST, `targets/${uuid}/baselines`, { stat, value: formElements.value.value, isMobile }).then(() => {
        if (callback) {
          callback();
        }
      });
      ...
      <Table.Root variant='surface' size='2' className='my-4'>
        <Table.Header>
          ...
          <Table.Row align='center'>
            ...
            {featureFlagEnabled('setBaselines') && <Table.ColumnHeaderCell>Update</Table.ColumnHeaderCell>}
          </Table.Row>
        </Table.Header>
        ...
        <Table.Body>
          {cwvStats.map((stat) => (
            <Table.Row>
              ...
              {featureFlagEnabled('setBaselines') && (
                <Table.Cell>
